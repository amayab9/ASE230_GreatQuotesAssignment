<?php
session_start();
?>
<!DOCTYPE HTML>
<html lang="eng">
<?php
		require("../util_csv.php");
		//function used to delete author
		function deleteAuthor($index)
		{
			dRow($index, 'authors.csv');
			$quotes = rFile('..\data\quotes.csv');
			$quoteExists = false;
			//loops through quotes
			foreach($quotes as $quote)
			{
				if ($quote[1] == $index)
				{
					$quoteExists = true;
				}
			}
			if ($quoteExists)
			{
				$j = 0;
				foreach($quotes as $quote)
				{
					if ($quote[1] == $index)
					{
						dRow($j, '..\data\quotes.csv');
						continue;
					}
					elseif ($quote[1] > $index)
					{
						$quote[1] = $quote[1] - 1;
						modRow($j, $quote, '..\data\quotes.csv');
					}
					$j++;
				}
			}
		}
		deleteAuthor($_GET['index']);
	?>
	<head>
		<title>Great Authors - Author Created</title>
		<meta name="viewport" content="width=device-width, initial-scale=1" charset="utf-8">
		<!-- CSS only -->
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Zenh87qX5JnK2Jl0vWa8Ck2rdkQ2Bzep5IDxbcnCeuOxjzrPF/et3URy9Bv1WTRi" crossorigin="anonymous">
	</head>
	<body>
		<!--Body Container-->
		<div style="height: 100%;">
			<!--Top Bar-->
			<div class="" style="height: 60px;">
				<div class="divTwo" style="width: 300px; height: 60px; float: left;">
                    <button type="button" class="btn btn-outline-primary">
                        <a class="butTwo" href="..\quotes\index.php">Switch to Quotes</a>
                    </button>
            </div>
				<!--displays sign out button if logged in-->
				<?php if(isset($_SESSION['logged']) && isset($_SESSION['logged_user'])) { ?>
				<div class="divTwo" style="width: 300px; height: 60px; float: left; background-color: red;">
                    <button type="button" class="btn btn-outline-success">
                        <a class="butTwo" href="../auth/signout.php">Sign Out</a>
                    </button>
                </div>
				<?php } ?>
				<?php if(!isset($_SESSION['logged']) || !isset($_SESSION['logged_user'])) { ?>
				<!--displays sign in button if not isn't logged in-->
				<div class="divThree" style="width: 300px; height: 60px; float: left;">
                    <button type="button" class="btn btn-outline-success">
                        <a class="butTwo" href="../auth/signin.php">Sign In</a>
                    </button>
                </div>
				<?php } ?>
			</div>
			<!--Small Top Bar-->
			<div class="lg" style="height: 5px;"></div>
			<!--Quote Column-->
			<div class="textColumn" style="min-height: 100%;">
				<p class="textlgb" style="font-size: 100px;">Author Created!</p>
				<!--Buttons Row-->
				<div class="row" style="padding-top: 30px;">
					<div class="col-md-2 col-sm-5 col-xs-12">
                        <div class="btnDiv">
                            <button type="button" class="btn btn-secondary">
                            <a class="btn" href="index.php">Home</a>
                            </button>
                        </div>
                    </div>
				</div>
			</div>
			<!--Small Footer Bar-->
			<div class="lg" style="height: 5px;"></div>
			<!--Footer Bar-->
			<div class="lb" style="height: 30px;"></div>
		</div>
	</body>
</html>
